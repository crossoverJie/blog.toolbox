    ...
    private void replacePic(String path, Map<String, String> picMapping) {
        ...
        for (Map.Entry<String, String> pic : picMapping.entrySet()) {
            ...
            try {
                ...
                log.error("替换内容失败，源=[{}]，目标=[{}]", oldPic, newPic, e);
            } finally {
                try {
                    out.close();
                    bufIn.close();
                    ...
                    log.error("替换文件关闭流失败，IOException", e);
                }
    ...
    private Map<String, String> downUpPic(Stream<String> stringStream, String filePath) {
        ...
        for (String pic : pics) {
            ...
            try {
                ...
                log.info("下载[{}]图片成功,地址=[{}]", pic, path);
            } catch (IOException e) {
                log.error("下载图片失败 fileName=[{}]", path, e);
            }
            ...
            try {
                ...
                if (uploadAddress == null) {
                    log.error("上传图片失败,跳过 fileName=[{}]", path);
                    continue;
                }

                log.info("【上传[{}]图片成功,地址=[{}]】", pic, uploadAddress);
                picMapping.put(pic, uploadAddress);
            } catch (Exception e) {
                log.error("上传图片失败 fileName=[{}],{}", path, e);
            }
